...
// Ilustração de cada card, na ordem dos cards editados no Studio.
const images = [illoChart, illoForklift, illoCalendar, illoDesktop];

type FleetTechContent = SectionContent<typeof locacaoPage.sections.fleetTech>;
...
export function FleetTechSection({ content }: { content: FleetTechContent }) {
  return (
    <Section className="flex flex-col gap-10 lg:gap-12">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {content.cards.map((card, i) => (
          <FleetCard key={card.title} {...card} image={images[i]} />
        ))}
      </div>
    </Section>
  );
